...
async def sync_data(
    ...
    for p in photos:
        ...
        photo_list.append(PhotoMetadata(
            ...
            has_preview=bool(p["preview_path"]) if "preview_path" in p.keys() else False,
...
async def search_places(
    ...
    - OpenStreetMap 기반 역지오코딩
    """
    if not q or len(q) < 2:
        raise HTTPException(status_code=400, detail="검색어는 2자 이상이어야 합니다")

    results = []
    nominatim_results = await exif_service.search_places(q, limit=limit)
